...
#define RANDOMIZER_MOVE_TYPE_WEIGHT_STAB 6
...
static u32 GetRandomizerStatusTierWeight(u32 moveTier, u8 level)
{
    if (level == 1)
    {
        static const u8 sInitialWeights[] = {20, 0, 0};
        return sInitialWeights[moveTier];
    }
    else if (level < 24)
    {
        static const u8 sEarlyWeights[] = {20, 12, 6};
        return sEarlyWeights[moveTier];
    }
    else if (level < 42)
    {
        static const u8 sMiddleWeights[] = {5, 48, 17};
        return sMiddleWeights[moveTier];
    }
    else
    {
        static const u8 sLateWeights[] = {2, 20, 72};
